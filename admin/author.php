...
if (isset($_POST["add_author"])) {
    ...
    if ($error == '') {
        ...
        $statement->execute(array(':author_name' => $formdata['author_name']));
        ...
        if ($statement->rowCount() > 0) {
            ...
            $query = "INSERT INTO lms_author (author_name, author_status, author_created_on) VALUES (:author_name, :author_status, :author_created_on)";
            $statement = $connect->prepare($query);
...
if (isset($_POST["edit_author"])) {
    ...
    if ($error == '') {
        ...
        $statement = $connect->prepare($query);
        $statement->execute(array(':author_name' => $formdata['author_name'], ':author_id' => $author_id));
        ...
        if ($statement->rowCount() > 0) {
            ...
            $query = "UPDATE lms_author SET author_name = :author_name, author_updated_on = :author_updated_on WHERE author_id = :author_id";
            $statement = $connect->prepare($query);
            $statement->execute($data);
...
if (isset($_GET["action"], $_GET["code"], $_GET["status"]) && $_GET["action"] == 'delete') {
    ...
    $query = "UPDATE lms_author SET author_status = :author_status, author_updated_on = :author_updated_on WHERE author_id = :author_id";
    $statement = $connect->prepare($query);
    $statement->execute($data);
...
<div class="container-fluid py-4" style="min-height: 700px;">
    ...
    if (isset($_GET["action"])) {
        if ($_GET["action"] == "add") {
            ...
            <ol class="breadcrumb mt-4 mb-4 bg-light p-2 border nav">
            ...
            if ($author_id > 0) {
                ...
                $statement = $connect->prepare($query);
                $statement->execute(array(':author_id' => $author_id));

                foreach ($statement->fetchAll() as $author_row) {
                    ?>
